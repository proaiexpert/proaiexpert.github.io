...
p = Path('docs/site-evolution/spline/proai-cube-semantic-brand-moment-r2/capture.mjs')
s = p.read_text()

# Runner-safe resolution. This changes only evidence raster size, never the runtime trajectory or semantic timing.
...
if old_view not in s or old_probe not in s:
    raise SystemExit('720p adaptation anchors missing')
s = s.replace(old_view, new_view, 1).replace(old_probe, new_probe, 1)

# Batch pure pose inspection into one browser IPC call. No state mutation is introduced.
...
# Keep baseline-equivalence methodology unchanged. Optimize only the owner-film loop by grouping
# the existing per-frame runtime mutations, diagnostics, render and JPEG readback into one evaluate().
...
p.write_text(s)
print('R2 capture patched: 720p + batched pose inspection + single IPC/render/readback per owner frame')
