# PROAI EXPERT HERO — CORRECTIVE STATIC ONLY — NEXT CHAT TASK
...
`SAME HERO + CLEANER SCALE + PRECISE 01–04 RAIL + RESULT + RICHER DEPTH/MATERIAL + BETTER OUTPUT LOGIC`
...
- same dark graphite + controlled cyan direction;
- same input → processing → outputs idea;
- same overall website, not a poster or new concept.
...
### A. Rebuild the 01–04 rail precisely

The current rail is visually weak and looks uneven/crooked.

Correct it so that:

- `01 / 02 / 03 / 04` follow one clean vertical alignment system;
...
The current C-object is slightly too large/heavy in the right half.

Reduce its perceived scale approximately `3–6%` while preserving geometry/perspective.

Goal:

- slightly more breathing room;
- cleaner relationship to the rail;
- calmer premium balance with the H1;
- no weak tiny object and no large reframe.

### C. Improve right-side object material/depth without concept drift

Keep the same object family and silhouette.

Improve:

- richer graphite/metal tonal separation;
- more convincing edge/specular behavior;
- restrained silver/cold-steel highlights;
- extremely subtle warm champagne/steel reflection on selected edges only;
- deeper internal cyan chamber;
- multiple perceived depth planes;
- stronger foreground/rear separation;
- cleaner contact shadow/floor integration/reflection;
- slightly richer local atmosphere around the object.

Warm reflection is not a new gold brand color.

Do not turn the object into jewelry, a reactor, portal, hardware rack, gaming asset, dashboard, or cyberpunk machine.

### D. Fix the signal/output logic

The current cyan route still reads too much like graphics laid on top of an image.

Improve the static depth cues so the viewer feels the signal belongs inside the object:

- reduce the impression of one flat glowing line;
- use occlusion/depth cues where appropriate;
- let cyan energy feel deeper inside the chamber;
- make the right-side outputs feel generated by the Core;
- keep outputs short, controlled, and physically aligned to 01–04;
- no long spaghetti cables or giant bus structure.

## 5. P1 — LIGHT OPTICAL POLISH ONLY

Review these only if they are visibly weak after the P0 correction:

- microcopy below the primary CTA;
- accountability line;
- top-right CTA optical polish.

Do not rewrite/reconceptualize them. This is not a copy pass.

## 6. DO NOT DO

The pass fails if any of the following happen:

- full Hero is regenerated/recomposed from scratch;
- H1/header/CTA/layout materially change;
- C-shape becomes a different object;
- object becomes a giant sci-fi machine;
- rail becomes a dashboard/infographic;
...
Before presenting the candidate, independently verify:

1. It is unmistakably the same Hero concept.
2. The C-object is only slightly smaller, not reframed/reinvented.
