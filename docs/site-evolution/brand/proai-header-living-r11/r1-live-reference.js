...
try{
  ...
  window.__PROAI_R1_REFERENCE_STATE={ready:true,mode,reducedMotion:reduced,glbUrl:GLB_URL,home:HOME,motion:MOTION,geometry:diagnostics.geometry||api.geometry,current:{yaw:HOME.yaw,pitch:HOME.pitch,roll:HOME.roll},elapsedSec:0};
  document.documentElement.dataset.r1Ready='true';
  if(mode==='living'&&!reduced){
    const t0=performance.now(),intervalMs=1000/MOTION.targetFps;
    const tick=()=>{const now=performance.now(),e=(now-t0)/1000,yaw=HOME.yaw+e*MOTION.yawSpeed,pitch=HOME.pitch+MOTION.pitchAmplitude*Math.sin(e*MOTION.pitchRate),roll=HOME.roll+MOTION.rollAmplitude*Math.sin(e*MOTION.rollRate);api.setSignatureEuler(yaw,pitch,roll,true);window.__PROAI_R1_REFERENCE_STATE.elapsedSec=e;window.__PROAI_R1_REFERENCE_STATE.current={yaw,pitch,roll}};
    tick();setInterval(tick,intervalMs);
  }
}catch(error){console.error('[ProAI R1 Reference]',error);document.documentElement.dataset.r1Ready='error'}finally{if(moduleUrl)setTimeout(()=>URL.revokeObjectURL(moduleUrl),0)}