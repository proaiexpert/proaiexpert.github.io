...
Logo R1 and Logo R2 symbol inventions are rejected and are not continued. R3 uses the existing approved ProAI Cube directly as the brand object.

Identity architecture: **ONE OBJECT · MULTIPLE BRAND STATES**.
...
Header/favicons/16px/28px/vector reduction/light-background production adaptation are intentionally deferred.
...
The source defines the 3×3 physical language from the canonical GLB transform hierarchy plus the established precision face/core replacement geometry. Geometry R1 values in the consumed source include face outer size `196.8`, face corner radius `10.6`, bevel size `2.35`, bevel thickness `1.25`, and core size `198.0` / radius `9.2`.
...
- `owner-review.html` — one-direction review board;
- `static-signature.html?capture` — large static premium signature;
- `static-lockup.html?capture` — practical object + wordmark lockup;
- `business-card.html?capture` — dark identity-only business-card scale test;
- `social-square.html?capture` — square/social identity test;
- `living-digital.html?capture` — living digital signature using the same Cube.

## Scope safety

All R3 additions are isolated under `docs/site-evolution/brand/proai-logo-canonical-cube-r3/` on the existing Logo review branch. No production Header, Hero, Homepage, Cube runtime, Footer, favicon, manifest, production CSS/JS, `main`, or active Cube development file is modified.
