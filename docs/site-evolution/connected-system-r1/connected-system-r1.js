(() => {
  ...
  /* ------------------------------------------------------------
     REVIEW-ONLY HEADER PARITY
     Mirrors the canonical mobile-behavior thresholds without
     modifying production Header assets.
     ------------------------------------------------------------ */
  const header=document.querySelector('[data-site-header]');
  const headerMobile=window.matchMedia('(max-width: 1200px), ((max-height: 540px) and (orientation: landscape))');
  ...
  const initReviewHeader=()=>{
    ...
    const openLabel=isRu?'Открыть меню':'Open menu';
    const closeLabel=isRu?'Закрыть меню':'Close menu';

    let lastScrollY=Math.max(0,window.scrollY||0);
    let ticking=false;
    let lastMode=headerMobile.matches;
    let lastOrientation=window.innerWidth>window.innerHeight?'landscape':'portrait';

    const isOpen=()=>!!(toggle&&nav&&(
      toggle.getAttribute('aria-expanded')==='true'||nav.classList.contains('is-open')
    ));
    ...
    const syncScroll=()=>{
      ...
      if(isOpen()||currentY<24||delta<-6){
        reveal();
      }else if(delta>8&&currentY>hideAfter){
    ...
    if(toggle&&nav){
      ...
      toggle.addEventListener('click',(event)=>{
        if(!headerMobile.matches)return;
      ...
      document.addEventListener('click',(event)=>{
        if(!headerMobile.matches||!isOpen()||header.contains(event.target))return;
        setOpen(false,false);
      });
    }
    ...
    window.addEventListener('pageshow',resetVisible);

    window.addEventListener('orientationchange',()=>{
      window.setTimeout(()=>{
        lastOrientation=window.innerWidth>window.innerHeight?'landscape':'portrait';
        lastMode=headerMobile.matches;
        resetVisible();
      },120);
    });

    window.addEventListener('resize',()=>{
      const nextMode=headerMobile.matches;
      const orientation=window.innerWidth>window.innerHeight?'landscape':'portrait';
      if(nextMode!==lastMode||orientation!==lastOrientation){
        lastMode=nextMode;
        lastOrientation=orientation;
        if(!nextMode)setOpen(false,false);
        resetVisible();
      }else{
        requestSync();
      }
    ...
    headerMobile.addEventListener?.('change',()=>{
      if(!headerMobile.matches)setOpen(false,false);
      resetVisible();
    });
  ...
  /* ------------------------------------------------------------
     CONNECTED SYSTEM R1.3
     One optical source coordinates rail, carriage, joint, title
     reflection and short transfer memory.
     ------------------------------------------------------------ */
  ...
  const replayMobile=()=>{
    ...
    stages.forEach((stage,i)=>{
      clearTransientClasses(stage);
      setState(i,mobileStateFor(i,index));
    });
    ...
    const targetRail=railPosition(index);
    const targetCarriage=carriagePosition(index);
    const startRail=targetRail-34;
    const startCarriage={x:targetCarriage.x,y:targetCarriage.y-42};

    if(focusWindow){
      focusAnimation=focusWindow.animate([
        {transform:focusTransform(startRail),opacity:.12},
        {transform:focusTransform(targetRail),opacity:.72}
      ],{duration:580,easing:'cubic-bezier(.2,.72,.18,1)',fill:'forwards'});
    }

    if(lightCarriage){
      carriageAnimation=lightCarriage.animate([
        {transform:carriageTransform(startCarriage),opacity:.22},
        {transform:carriageTransform(targetCarriage),opacity:.78}
      ],{duration:580,easing:'cubic-bezier(.2,.72,.18,1)',fill:'forwards'});
    }

    later(()=>{
      activateStage(index);
      placeOptical(index,.60,.74);
