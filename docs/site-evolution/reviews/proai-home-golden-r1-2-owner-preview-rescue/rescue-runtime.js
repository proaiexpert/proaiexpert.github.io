(()=>{
const PRODUCT='d5f562ac4c0c84b4ec06e913ec7e0f82d133beab';
...
function renderFooter(t){
 ...
 const f={
  '{{ footer_copy.home_href }}':cfg.home,'{{ footer_copy.home_label }}':cfg.homeLabel,'{{ footer_copy.social_label }}':cfg.socialLabel,'{{ footer_copy.locale_label }}':cfg.localeLabel,'{{ footer_copy.locale_lang }}':cfg.localeLang,'{{ footer_copy.locale_text }}':cfg.localeText,'{{ footer_copy.copyright }}':cfg.copyright,
  "{% if footer_lang == 'ru' %}/{% else %}/ru/{% endif %}":cfg.localeHref
 };
...
Promise.all([
 ...
 if(!head.includes('homepage-two-worlds-golden-r1-landscape-fix.css')||!head.includes('home-footer')&&false)throw new Error('Exact head authority missing');
 ...
 const p=source.indexOf(footerMarker);if(p<0)throw new Error('Footer marker missing from exact source');
 ...
 const banned=['a876dfb178fd3f020c8392b336eae62253f89ae7','home-footer-watermark-r2','owner-preview-qa-pending','proai-golden-r12-downstream','{%'];
 for(const x of banned)if(s.includes(x))throw new Error('Stale marker: '+x);
 const required=[PRODUCT,'homepage-two-worlds-golden-r1-landscape-fix.css','home-technology-transition-r2-golden-mobile.css','home-selected-thinking-r1-2.css','home-selected-work-golden-r1-2.css','home-footer-golden-r3-1.css','PROAI EXPERT','Financial Stream','Local Repair Pro'];
 for(const x of required)if(!s.includes(x))throw new Error('Required marker missing: '+x);
 document.open();document.write(s);document.close();
}).catch(e=>{console.error(e);document.body.innerHTML=`<pre style="white-space:pre-wrap;padding:24px">OWNER PREVIEW BLOCKED: ${String(e.message||e)}</pre>`});
})();
