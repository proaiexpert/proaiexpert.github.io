...
function updateSemanticOpticalState(mesh,index,presence,opportunity,release){const s=semanticOpticalScratch,q=mesh.getWorldQuaternion(s.q),center=mesh.getWorldPosition(s.center),cameraWorld=camera.getWorldPosition(s.camera),lightWorld=key.getWorldPosition(s.light),normal=s.normal.set(0,0,1).applyQuaternion(q).normalize(),tx=s.tx.set(1,0,0).applyQuaternion(q).normalize(),ty=s.ty.set(0,1,0).applyQuaternion(q).normalize(),view=s.view.copy(cameraWorld).sub(center).normalize(),lightDir=s.lightDir.copy(lightWorld).sub(center).normalize(),half=s.half.copy(view).add(lightDir).normalize();const faceView=THREE.MathUtils.clamp(normal.dot(view),0,1),halfDot=THREE.MathUtils.clamp(normal.dot(half),0,1),halfAlignment=smootherstep(THREE.MathUtils.clamp((halfDot-.46)/.50,0,1)),viewSupport=.88+.12*smootherstep(THREE.MathUtils.clamp((faceView-.52)/.42,0,1)),alignment=(.58+.42*halfAlignment)*viewSupport;let ax=lightDir.dot(tx),ay=lightDir.dot(ty),alen=Math.hypot(ax,ay);if(alen<.08){ax=.82;ay=.57;alen=Math.hypot(ax,ay)}s.axis.set(ax/alen,ay/alen);const hx=THREE.MathUtils.clamp(half.dot(tx),-.58,.58),hy=THREE.MathUtils.clamp(half.dot(ty),-.58,.58);s.fieldCenter.set(.5+hx*.13,.5+hy*.11);const soft=index===0,state=mesh.material.userData.semanticOpticalState;copySemanticOpticalState(state,{presence,opportunity:opportunity*(1-release*.70),alignment,baseAlpha:soft?SEMANTIC_R4_2_OPTICAL.baseAlphaSoft:SEMANTIC_R4_2_OPTICAL.baseAlphaSharp,eventAlpha:soft?SEMANTIC_R4_2_OPTICAL.eventAlphaSoft:SEMANTIC_R4_2_OPTICAL.eventAlphaSharp,center:s.fieldCenter,axis:s.axis});const opticalMoment=THREE.MathUtils.clamp(opportunity*alignment*(1-release*.80),0,1),shadow=new THREE.Color(SEMANTIC_R4_MATERIAL_PALETTE.shadowMetal),mid=new THREE.Color(SEMANTIC_R4_MATERIAL_PALETTE.midSilver),pearl=new THREE.Color(SEMANTIC_R4_MATERIAL_PALETTE.pearlSilver),c=shadow.clone().lerp(mid,(soft?.10:.16)+opticalMoment*(soft?.12:.20));c.lerp(pearl,opticalMoment*(soft?.06:.16));const m=mesh.material;m.color.copy(c);m.metalness=THREE.MathUtils.lerp(soft?.56:.64,soft?.62:.71,opticalMoment);m.roughness=THREE.MathUtils.lerp(soft?.60:.52,soft?.50:.38,opticalMoment);m.clearcoat=THREE.MathUtils.lerp(soft?.012:.025,soft?.035:.060,opticalMoment);m.clearcoatRoughness=THREE.MathUtils.lerp(.39,soft?.35:.30,opticalMoment);m.envMapIntensity=THREE.MathUtils.lerp(soft?.58:.70,soft?.75:.98,opticalMoment);m.specularIntensity=THREE.MathUtils.lerp(soft?.42:.54,soft?.56:.72,opticalMoment);syncSemanticOpticalUniforms(m);if(index===1)semanticR42OpticalDiagnostics={alignment,faceView,halfDot,fieldCenter:s.fieldCenter.toArray(),fieldAxis:s.axis.toArray(),opportunity,presence,release}}
function setSemanticOpticalState(mesh,index,presence,opportunity,release){updateSemanticOpticalState(mesh,index,presence,opportunity,release)}`;
