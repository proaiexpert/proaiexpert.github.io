...
const material = `const SEMANTIC_R4_MATERIAL_PALETTE = Object.freeze({shadowMetal:'#242A31',midSilver:'#687077',pearlSilver:'#B8BBB8',peakReflection:'#D7D8D4'});
const SEMANTIC_R4_3_ENGRAVED_FACE = '+Z';
const SEMANTIC_R4_3_ENGRAVING = Object.freeze({bumpScale:-0.032,baseRoughness:.295,baseMetalness:.84,baseEnv:.18+1.0,baseClearcoat:.16,baseClearcoatRoughness:.20,driver:'persistent bump-derived micro-normal + orientation-gated microfacet roughness response from face normal + camera view + stable key RectAreaLight half-vector + surface-space field'});
...
let semanticR43OpticalDiagnostics={alignment:0,faceView:0,halfDot:0,fieldCenter:[.5,.5],fieldAxis:[1,0],opportunity:0,persistent:true,microNormal:true,alphaDominant:false,engravedFace:SEMANTIC_R4_3_ENGRAVED_FACE,materialPropertiesStatic:true};
...
one('    textMeshCount: semanticTextMeshes.length,\n', `    textMeshCount: semanticTextMeshes.length,
    ...
    revealCharacter: 'persistent engraving / orientation-driven microfacet legibility / no reveal animation',
    opticalDriver: SEMANTIC_R4_3_ENGRAVING.driver,
    opticalField: semanticR43OpticalDiagnostics,
    baseInlayTreatment: 'permanent +Z graphite-matched micro-engraving; same base face material response at idle, bump-derived edge normal always present',
...
console.log(JSON.stringify({material:'MeshPhysicalMaterial / R4.3 persistent physical micro-engraving',engravedFace:'+Z',surfaceMaxOpacity:0,persistentInscription:true,alphaDominantReveal:false,microNormalEngraving:true,materialPropertiesStatic:true,opticalDriver:'persistent bump-derived micro-normal + orientation-gated microfacet roughness response from face normal + camera view + stable key RectAreaLight half-vector + surface-space field',baseMaterialMatch:'graphiteFace #242A31 / metalness .84 / roughness .295 / clearcoat .16 / clearcoatRoughness .20 / envMapIntensity 1.18',seamAwareOcclusion:true,emissive:false,glowHalo:false,scanWipeShimmer:false,letterWordStagger:false,semanticVelocityMultiplier:1,semanticAcceleration:false,semanticDeceleration:false,semanticOrientationForcing:false,semanticMotionCoupled:false,effectiveGlobalMotionScale:1,timing:{triggerSearchStartMs:3200,triggerSearchEndMs:4200,opportunityEnvelopeMs:2370},motionChangedBySemantic:false},null,2));
