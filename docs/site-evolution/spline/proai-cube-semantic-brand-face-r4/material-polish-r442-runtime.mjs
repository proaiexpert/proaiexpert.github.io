...
// First discovery arms 3.4s after load; the 2.40s maximum is only a safety cap,
...
replaceUnique('maxProtectedMs:3200','maxProtectedMs:2400','maximum physical readability safety cap');
...
for(const required of[
 ...
 'approachScore:.58,enterScore:.76,exitScore:.54,minView:.58,exitView:.50','maxProtectedMs:2400','nextEligiblePresentationMs:R44_INITIAL_PRESENTATION_PHASE_MS+3400','rearmScore:.50',
...
for(const forbidden of['createSemanticR44PlanarFaceGeometry(mesh,mesh.geometry,face)','createSemanticR44BevelTile(maskTexture)','createSemanticR44ToneTile(maskTexture)','maxProtectedMs:3200','maxProtectedMs:2480','nextEligiblePresentationMs:2600','unsafeReturnGuardMs:3000'])if(source.includes(forbidden))throw new Error(`R4.4.2 unresolved legacy dependency: ${forbidden}`);

fs.writeFileSync(file,source);
console.log('R4.4.2 final 2.40s readability safety-cap + optical re-arm + axis diversity applied');
