...
const R3_DEFAULT = path.join(repoRoot, 'owner-preview/assets/3d/boxes-hover/neutral-light-material-r3-final.bin');
const GOLDEN_DEFAULT = path.join(repoRoot, 'owner-preview/assets/3d/boxes-hover/public-original-inline-scene-payload.bin');
const DEFAULT_OUTPUT = path.join(repoRoot, 'docs/site-evolution/ai-systems/hero-r3-rest-visibility-r1/candidates');

const RECIPES = {
  'candidate-a': {
    label: 'Candidate A · exact RGB midpoint R3 Micro 2 ↔ Micro 3 ceiling',
    colors: [
      [(0.5882352941176471 + 0.6470588235294118) / 2, (0.6078431372549019 + 0.6627450980392157) / 2, (0.6235294117647059 + 0.6784313725490196) / 2, 1],
      [(0.5686274509803921 + 0.611764705882353) / 2, (0.5882352941176471 + 0.6392156862745098) / 2, (0.6039215686274509 + 0.6549019607843137) / 2, 1],
      [(0.7725490196078432 + 0.8156862745098039) / 2, (0.788235294117647 + 0.8274509803921568) / 2, (0.803921568627451 + 0.8392156862745098) / 2, 1],
    ],
  },
  'candidate-b': {
    label: 'Candidate B · exact Micro 3 ceiling',
    colors: [
...
function parseArgs() {
const options = { payload: R3_DEFAULT, golden: GOLDEN_DEFAULT, output: DEFAULT_OUTPUT, name: 'candidate-a', recipe: 'candidate-a' };
...
function main() {
  ...
  const r3Bytes = fs.readFileSync(options.payload);
  const goldenSha = sha256(goldenBytes);
  const r3Sha = sha256(r3Bytes);
  if (goldenSha !== GOLDEN_SHA) throw new Error(`Golden SHA mismatch: ${goldenSha}`);
  if (r3Sha !== R3_SHA) throw new Error(`R3 SHA mismatch: ${r3Sha}`);
  const goldenRoot = decodeOne(msgpackr, goldenBytes);
  const r3Root = decodeOne(msgpackr, r3Bytes);
  const r3Json = JSON.parse(JSON.stringify(r3Root));
  const ownership = ownershipMap(r3Root);
  const originalGradient = findGradientLayers(r3Root).map((item) => ({ materialPath: item.materialPath, layerIndex: item.layerIndex, layerId: item.layerId, alpha: item.data.alpha, mode: item.data.mode, gradientType: item.data.gradientType, colors: item.data.colors, steps: item.data.steps, num: item.data.num, angle: item.data.angle, offset: item.data.offset, morph: item.data.morph }));
  const candidateRoot = decodeOne(msgpackr, r3Bytes);
  ...
  const changed = leafDiff(r3Json, candidateJson);
  ...
  const roundtripChanged = leafDiff(r3Json, JSON.parse(JSON.stringify(decodedCandidate)));
  ...
  const evidence = {
    authority: { goldenSha256: goldenSha, r3Sha256: r3Sha, goldenBytes: goldenBytes.length, r3Bytes: r3Bytes.length, runtime: '@splinetool/runtime@2.0.27', encoder: 'public Packr({structuredClone:true}) plus public extension table' },
    recipe: { name: options.recipe, label: recipe.label, colors: recipe.colors },
    ownership,
    originalGradient,
    mutation,
    r3CubeMaterial: cubeSnapshot(r3Root),
    ...
    safety: { goldenModified: false, r3Modified: false, runtimeMutation: false, cubeMaterialModified: false, geometryModified: false, cameraModified: false, hierarchyModified: false, hoverModified: false },
  };
