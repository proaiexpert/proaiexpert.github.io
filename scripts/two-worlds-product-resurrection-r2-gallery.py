...
for c in manifest['ownerGalleryCandidates']:
    ...
    ov=r['bodyOverflow']
    ov_text=' / '.join(f"{k}: {v['delta']}px" for k,v in ov.items())
    failures='NONE' if not r['assetFailures'] else html.escape(json.dumps(r['assetFailures']))
    css='<br>'.join(html.escape(x) for x in r['loadedCss']) or 'NONE'
    js='<br>'.join(html.escape(x) for x in r['loadedJs']) or 'NONE'
    cards.append(f'''
<section class="candidate" id="candidate-{cid.lower()}">
  ...
  <div class="meta">
    ...
    <div><b>BODY OVERFLOW DELTA</b><span>{ov_text}</span></div>
    <div><b>REQUIRED ASSET FAILURES</b><span>{failures}</span></div>
    <div><b>LOADED TWO WORLDS CSS</b><span class="mono">{css}</span></div>
    <div><b>LOADED TWO WORLDS JS</b><span class="mono">{js}</span></div>
  </div>
