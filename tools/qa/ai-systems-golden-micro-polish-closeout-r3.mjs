...
const out = process.env.GITHUB_WORKSPACE + '/qa-evidence-closeout-r3';
fs.mkdirSync(out,{recursive:true});
const cases = [
  ['EN DESKTOP','en',process.env.EN_URL,1440,1100],
  ['RU DESKTOP','ru',process.env.RU_URL,1440,1100],
  ['EN 390','en',process.env.EN_URL,390,844],
  ['RU 390','ru',process.env.RU_URL,390,844],
  ['EN 844x390','en',process.env.EN_URL,844,390],
  ['RU 844x390','ru',process.env.RU_URL,844,390]
];
const expected = {
...
for (const [label,lang,url,width,height] of cases) {
  ...
  for (const [groupName,selector,type] of groups) {
    ...
    for (const idx of indices) {
      ...
      await page.$$eval(selector,(els,{idx,qaId})=>els[idx]?.setAttribute('data-qa-closeout',qaId),{idx,qaId});
      const {nodeId}=await cdp.send('DOM.querySelector',{nodeId:root.nodeId,selector:`[data-qa-closeout="${qaId}"]`});
      ...
      for (const state of states) {
        ...
        if(groupName==='Hero primary'||groupName==='Section 08 CTA'){
          ...
          const l=lum(rgb(s.color));
          ck(l!==null&&l>0.6,`${groupName} ${state} text=${s.color}`);
        }
  ...
  await cdp.detach();
  fs.writeFileSync(`${out}/${label.replace(/[^a-z0-9]/gi,'_')}.states.json`,JSON.stringify(stateRows,null,2));
  await page.screenshot({path:`${out}/${label.replace(/[^a-z0-9]/gi,'_')}.png`,fullPage:false});
  await page.close();
  failures.push(...f.map(x=>`${label}: ${x}`));
  summaries.push({label,pass:f.length===0,http,overflow:core.overflow,missingCss:resourceFailures.filter(x=>x.type==='stylesheet').length,missingJs:resourceFailures.filter(x=>x.type==='script').length,consoleErrors:consoleErrors.length,requestFailures:requestFailures.length,pills:core.pills.length,stateCount:stateRows.length});
  console.log(`CASE ${label} ${f.length?'FAIL':'PASS'} HTTP=${http} OVERFLOW=${core.overflow} MISSING_CSS=${resourceFailures.filter(x=>x.type==='stylesheet').length} MISSING_JS=${resourceFailures.filter(x=>x.type==='script').length} CONSOLE=${consoleErrors.length} REQUEST=${requestFailures.length} PILLS=${core.pills.length} STATES=${stateRows.length}`);
  f.forEach(x=>console.log(`FAIL ${label}: ${x}`));
}

await browser.close();
const report={pass:failures.length===0,summaries,failures};
fs.writeFileSync(`${out}/closeout-report.json`,JSON.stringify(report,null,2));
